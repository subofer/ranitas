...
export default async function buscarPorCodigoDeBarras(texto) {
  ...
  const navegador = await getBrowserInstance(true);
  const pagina = await navegador.newPage();
  await pagina.goto(url);

  const evaluation = await pagina.evaluate((codigoDeBarras) => {
    ...
    divsBusqueda.forEach((div, index) => {
      ...
      let precioString = Array.from(spans)
      ...
      const regexUnidades = /(\d+)\s?(ml|cc|kg|gr|g)\b/gi;
      ...
      if (resultadoUnidades && resultadoUnidades.length > 0) {
        ...
        if (match) {
          detalles.cantidad = parseInt(match[1], 10);
  ...
  evaluation.forEach((r,i) => {
    let puntaje = 0;
    if (r.imagen.tieneImagen) puntaje += 30;
    if (r.detalles.tieneUnidad) puntaje += 20;
    if (r.precio.enPesosArgentinos) puntaje += 10;
    if (r.precio.valor == 0) puntaje -= 5;
    if (r.rating.tieneRating) puntaje += 1;
  ...
  if (pRis.length != 0){
    evaluation.forEach(r => {
      if (!r.imagen.tieneImagen || r.imagen.src == "ImagenTruchaCambiar") {
        lastUsed = dr(lastUsed, pRis); r.imagen = pRis[lastUsed].imagen;
      }
    });
  }

  //Cerrar pagina y navegador.
  pagina.close();
  navegador.close();

  return {
    resultadosDeLaBusqueda: evaluation,
    primerResultadoDeLaBusqueda: evaluation[0],
  };
}
