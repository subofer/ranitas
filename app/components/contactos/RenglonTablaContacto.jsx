...
const tipoRender = ({esProveedor, esMarca, esInterno}) => {
  return (
    <div className="w-fit flex flex-row gap-1 flex-wrap justify-center mx-auto text-balance ">
      {esInterno && <Phill className={`pb-1.5`}>Interno</Phill>}
      {esMarca && <Phill className={`pb-1.5`}>Marca</Phill>}
      {esProveedor && <Phill className={`pb-1.5`}>Proveedor</Phill> }
    </div>
  )
}

export const RenglonTablaContacto = ({item: contacto, className}) => {
  const { addParam, recarga } = useMyParams();

  const editar = useCallback(() => {
    addParam("cuit", contacto.cuit)
  },[addParam, contacto.cuit])
  ...
  return (
    <Tr className={`h-[60px] text-center  ${className} `}>
      <Td className="w-[80px]">
        {textos.resumen(contacto?.id)}
      </Td>
      <Td className="w-[8rem]">
        {textos.cuit(contacto?.cuit)}
      </Td>
      <Td className="w-[250px]">
        {contacto?.nombre}
      </Td>
      <Td className="w-[200px]">
        {emailRenderer(contacto.emails)}
      </Td>
      <Td className="flex flex-col justify-center mx-auto w-[10px]">
        {tipoRender(contacto)}
      </Td>
