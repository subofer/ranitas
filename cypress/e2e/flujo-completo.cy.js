describe('Flujo Completo del Sistema', () => {
  before(() => {
    // Limpiar datos de prueba antes de todo el flujo
    cy.cleanupTestData()
  })

  it('debería completar un flujo completo: producto → proveedor → venta', () => {
    // 1. LOGIN
    cy.login('subofer', '1234')

    // 2. CREAR CATEGORÍA
    const categoriaNombre = `Categoría Test ${Date.now()}`
    cy.createTestCategory(categoriaNombre)
    cy.contains(categoriaNombre).should('be.visible')

    // 3. CREAR PROVEEDOR
    const proveedorCuit = '30123456780'
    const proveedorNombre = `Proveedor Test ${Date.now()}`
    cy.createTestSupplier(proveedorCuit, proveedorNombre)
    cy.contains(proveedorNombre).should('be.visible')

    // 4. CREAR PRODUCTO
    const productoCodigo = `TEST${Date.now()}`
    const productoNombre = `Producto Completo ${Date.now()}`
    cy.visit('/cargarProductos')

    cy.get('input[name="codigoBarra"]').type(productoCodigo)
    cy.get('input[name="nombre"]').type(productoNombre)
    cy.get('input[name="descripcion"]').type('Producto creado en test de flujo completo')

    // Seleccionar categoría
    cy.contains('Seleccionar Categorías').click()
    cy.contains(categoriaNombre).click()

    // Seleccionar proveedor
    cy.contains('Seleccionar Proveedores').click()
    cy.contains(proveedorNombre).click()

    // Ingresar precio
    cy.get('input[name="precio"]').type('100.50')
    ...
    cy.get('input[placeholder*="Buscar"]').type(productoNombre.substring(0, 10))
    cy.contains(productoNombre).should('be.visible')
