...
async function makeSafeImageForQwen(buffer, minSize = 672, maxSize = 896) {
  try {
    ...
    const targetSide =
      Math.ceil(Math.max(minSize, Math.max(scaledW, scaledH)) / 28) * 28;
...
async function optimizeImageForAI(imageBuffer) {
  try {
    ...
    if (
      optimizedBase64.includes("data:") ||
      optimizedBase64.includes(";base64,")
    ) {
      throw new Error("Base64 contiene prefijos inválidos");
    }

    const reduction = (
      (1 - optimizedBase64.length / originalBase64.length) *
      100
    ).toFixed(1);
...
function simpleInvoiceParserJS(text) {
  ...
  const parsed = {
    emisor: { nombre: "", cuit: "" },
    documento: { numero: "", fecha: null, totales: {} },
    items: [],
  };
...
function validateParsedInvoice(parsed) {
  ...
  if (
    ...
    errors.push("emisor_nombre_invalid");
  const cuit = parsed.emisor?.cuit || "";
  if (cuit && !/^(\d{2}-\d{8}-\d|\d{11})$/.test(String(cuit).trim()))
    errors.push("cuit_bad_format");
  const numero = parsed.documento?.numero || "";
  if (!numero || String(numero).trim().length < 2)
    errors.push("documento_numero_invalid");
  if (
    !parsed.items ||
    !Array.isArray(parsed.items) ||
    parsed.items.length === 0
  )
    errors.push("items_missing");
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          metadata: {
            timing: { totalMs: tNow - tStart, human: `${tNow - tStart}ms` },
          },
        },
        { status: 400 },
      );
    ...
    if (action === 'warp') {
      ...
      try {
        ...
        if (!pointsParam) {
          throw new Error('Se requieren puntos para enderezar la imagen');
        }
        ...
        if (!visionResponse.ok) {
          ...
          throw new Error(errorData.detail || 'Error en enderezado de imagen');
        }
        ...
        return NextResponse.json({
          ok: true,
          enhanced: `data:image/jpeg;base64,${visionData.image}`,
          metadata: {
            timing: { totalMs: tEnd - tStart, human: `${tEnd - tStart}ms` },
          },
        });
      ...
      if (!resp.ok) {
        ...
        if (/cannot identify image file/i.test(text)) {
          ...
          try {
            ...
            let fallbackBlob;
            try {
              fallbackBlob = new Blob([originalBuffer], {
                type: image.type || "image/jpeg",
              });
            } catch (e) {
              fallbackBlob = originalBuffer;
            }
            try {
              fallbackForm.append(
                "image",
              ...
              fallbackForm.append("image", fallbackBlob);
            }
            ...
            let fallbackResp;
            try {
              fallbackResp = await fetch(`${VISION_HOST}${visionEndpoint}`, {
                method: "POST",
                body: fallbackForm,
              });
            } catch (frErr) {
            ...
            if (fallbackResp && fallbackResp.ok) {
              ...
              return NextResponse.json(
                {
                  ok: false,
                  error: "Vision microservice error",
                  details: text,
                },
                { status: 502 },
              );
        ...
        if (jsonMatch) {
          ...
          if (mode === "factura" && parsedData) {
            ...
            if (parsedData.documento) {
              if (
                parsedData.documento.número_completo &&
                !parsedData.documento.numero
              ) {
                parsedData.documento.numero =
                  parsedData.documento.número_completo;
              }
            ...
            if (parsedData.totales) {
              ...
              for (const k of discountKeys) {
                if (
                  parsedData.totales[k] !== undefined &&
                  parsedData.totales[k] !== null &&
                  parsedData.totales[k] !== ""
                ) {
              ...
              if (
                ...
                Array.isArray(parsedData.totales.descuentos)
              ) {
                const sumFromArray = parsedData.totales.descuentos.reduce(
                  (s, x) => s + (getNumber(x.monto) || getNumber(x.valor) || 0),
                  0,
                );
              ...
              if (
                ...
                const sumItemsDiscount = parsedData.items.reduce(
              ...
              if (
                parsedData.totales.impuestos &&
                Array.isArray(parsedData.totales.impuestos)
              ) {
                // Buscar entradas que contengan 'iva' en su nombre
                const ivaItems = parsedData.totales.impuestos.filter(
                  (i) =>
                    /iva/i.test(i.nombre || "") ||
                    /iva/i.test(String(i.tipo || "")),
                );
                if (ivaItems.length > 0) {
                  parsedData.totales.iva = ivaItems.reduce(
                    (s, x) => s + (getNumber(x.monto) || 0),
                    0,
                  );
                }

                // Calcular impuestos_total como suma de montos si no viene explícito
                const impuestosSum = parsedData.totales.impuestos.reduce(
                  (s, x) => s + (getNumber(x.monto) || getNumber(x.valor) || 0),
                  0,
                );
                if (
                  (parsedData.totales.impuestos_total === undefined ||
                    parsedData.totales.impuestos_total === null) &&
                  impuestosSum
                ) {
              ...
              parsedData.totales.total_impreso =
                getNumber(parsedData.totales.total_impreso) !== undefined
                  ? getNumber(parsedData.totales.total_impreso)
                  : undefined;
              ...
              if (Array.isArray(parsedData.items)) {
                const sumNeto = parsedData.items.reduce((s, it) => {
                  ...
                  const lineRaw =
                    it.subtotal_calculado ??
                    it.subtotal_original ??
                    it.subtotal ??
                    unit * qty;
                ...
                const devolucionesSum = parsedData.items.reduce((s, it) => {
                  ...
                  const isDevol =
                    !!it.es_devolucion || !!it.devolucion || line < 0;
                ...
                if (
                  (descuentoTicket === undefined || descuentoTicket === null) &&
                  Array.isArray(parsedData.totales.descuentos)
                ) {
                  const sumFromArray = parsedData.totales.descuentos.reduce(
                    (s, x) =>
                      s + (getNumber(x.monto) || getNumber(x.valor) || 0),
                    0,
                  );
                ...
                parsedData.totales.descuento_ticket = Number(
                  descuentoTicket || 0,
                );
                parsedData.totales.descuento_total = Math.abs(
                  Number(parsedData.totales.descuento_ticket || 0),
                );

                const impuestos_total = Number(
                  parsedData.totales.impuestos_total ??
                    parsedData.totales.iva ??
                    0,
                );
                const recargos = Number(parsedData.totales.recargos_total ?? 0);

                // Calcular total_calculado en backend de forma determinista
                const totalCalculadoComputed = Number(
                  (
                    sumNeto -
                    parsedData.totales.descuento_total +
                    recargos +
                    impuestos_total
                  ).toFixed(2),
                );
                ...
                if (
                  getNumber(parsedData.totales.total_calculado) !== undefined
                ) {
                  parsedData.totales.total_calculado_model = getNumber(
                    parsedData.totales.total_calculado,
                  );
                }

                parsedData.totales.total_calculado = totalCalculadoComputed;
                parsedData.totales.sugerido = parsedData.totales.sugerido || {};
                parsedData.totales.sugerido.total_calculado =
                  totalCalculadoComputed;
              }
              ...
              if (typeof parsedData.totales.total_impreso === "number") {
                const tc =
                  getNumber(parsedData.totales.total_calculado) !== undefined
                    ? getNumber(parsedData.totales.total_calculado)
                    : parsedData.totales.sugerido?.total_calculado;
                if (typeof tc === "number") {
                  parsedData.totales.sugerido =
                    parsedData.totales.sugerido || {};
                  parsedData.totales.sugerido.diferencia = Number(
                    (parsedData.totales.total_impreso - tc).toFixed(2),
                  );
                  if (Math.abs(parsedData.totales.sugerido.diferencia) > 0.01) {
                    parsedData.totales.revisar = true;
                    parsedData.totales.sugerido.detalle =
                      parsedData.totales.detalle_diferencia ||
                      "Total impreso distinto a total calculado (sugerido)";
                  }
                }
              }
            ...
            if (parsedData.totales_calculados) {
              parsedData.totales_calculados.neto = fixArgentineNumber(
                parsedData.totales_calculados.neto,
              );
              parsedData.totales_calculados.iva = fixArgentineNumber(
                parsedData.totales_calculados.iva,
              );
              parsedData.totales_calculados.total = fixArgentineNumber(
                parsedData.totales_calculados.total,
              );
              console.log(
                "🧮 Totales calculados normalizados:",
                parsedData.totales_calculados,
              );
            }
            ...
            if (parsedData.items && Array.isArray(parsedData.items)) {
              ...
              for (let idx = 0; idx < parsedData.items.length; idx++) {
                ...
                const subtotal =
                  item.subtotal || item.importe || item.total || 0;
                ...
                try {
                  ...
                  const isDevol =
                    /devolu|devoluci|devoluci\w*|dev\b|devolución|devolución/i.test(
                      descLower,
                    );
                  ...
                  if (isDevol) {
                    ...
                    try {
                      const unit =
                        Number(
                          normalized.precio_unitario || normalized.precio || 0,
                        ) || 0;
                      const qty =
                        Number(
                          normalized.cantidad ||
                            normalized.cantidad_documento ||
                            0,
                        ) || 0;
                      const rawSubtotal = Number(normalized.subtotal || 0) || 0;
                      const assumed =
                        Math.abs(rawSubtotal) || Math.abs(unit * qty) || 0;
                      ...
                      console.warn(
                        "⚠️ Error al normalizar subtotales de devolución:",
                        subtotalErr.message,
                      );
                    }
                    ...
                    cleaned = cleaned.replace(
                      /\bdevolu\w*\b|\bdev\b|\bdevolución\b/gi,
                      "",
                    );
                    cleaned = cleaned.replace(
                    ...
                    try {
                      ...
                      try {
                        const aliasMatch =
                          await prisma.proveedorSkuAlias.findFirst({
                            where: {
                              OR: [
                                {
                                  nombreEnProveedor: {
                                    contains: normalized.descripcion_limpia,
                                    mode: "insensitive",
                                  },
                                },
                                {
                        ...
                        if (aliasMatch) {
                          normalized.productoInferido = aliasMatch.producto
                            ? {
                                id: aliasMatch.producto.id,
                                nombre: aliasMatch.producto.nombre,
                              }
                            : normalized.productoInferido;
                          if (aliasMatch.presentacion)
                            normalized.presentacionInferida = {
                              id: aliasMatch.presentacion.id,
                              nombre: aliasMatch.presentacion.nombre,
                            };
                          normalized.proveedorAlias = {
                            proveedorId: aliasMatch.proveedorId,
                            proveedorNombre:
                              aliasMatch.proveedor?.nombre || null,
                            sku: aliasMatch.sku || null,
                            nombreEnProveedor:
                              aliasMatch.nombreEnProveedor || null,
                            confidence: 0.95,
                          };
                          console.log(
                            "🔎 Match por alias de proveedor encontrado:",
                            normalized.proveedorAlias,
                          );
                        }
                      } catch (aliasErr) {
                        console.warn(
                          "⚠️ Error buscando alias de proveedor:",
                          aliasErr.message,
                        );
                      }
                      ...
                      if (!normalized.productoInferido) {
                        ...
                        if (producto) {
                          ...
                          const descForMatch =
                            normalized.descripcion_limpia.toLowerCase();
                          for (const p of producto.presentaciones || []) {
                            if (!p.nombre) continue;
                            if (
                              descForMatch.includes(p.nombre.toLowerCase()) ||
                              p.nombre.toLowerCase().includes(descForMatch)
                            ) {
                              presentacionMatch = p;
                              break;
                            }
                          ...
                          if (
                            ...
                            presentacionMatch =
                              producto.presentaciones.find((p) =>
                                /kg|g|l|ml|pack|unid|unidad/i.test(p.nombre),
                          ...
                          if (presentacionMatch)
                            normalized.presentacionInferida = {
                              id: presentacionMatch.id,
                              nombre: presentacionMatch.nombre,
                            };

                          // Para devoluciones: acción sobre stock
                          normalized.stockAction = "in"; // devolución aumenta stock
                          normalized.stockDelta = Math.abs(
                            Number(normalized.cantidad) || 0,
                          );

                          console.log(
                            "🔎 Devolución detectada - producto inferido:",
                            normalized.productoInferido,
                            "presentacion:",
                            normalized.presentacionInferida,
                          );
                          ...
                          console.log(
                            "🔎 Devolución detectada pero no se pudo inferir producto:",
                            normalized.descripcion_limpia,
                          );
                        }
                      }
                    } catch (dbErr) {
                      console.warn(
                        "⚠️ Error consultando DB para inferir producto de devolución:",
                        dbErr.message,
                      );
                    }
                  }
                  ...
                  console.warn(
                    "⚠️ Error detectando devoluciones:",
                    errDevol.message,
                  );
                }
              ...
              try {
                const rawOcr =
                  vdata && vdata.ocr_text
                    ? vdata.ocr_text
                    : data?.vision_meta?.ocr_text || responseText || "";
                try {
                  if (
                    (!parsedData.emisor || !parsedData.emisor.cuit) &&
                    rawOcr
                  ) {
                    ...
                    if (cuitMatch) {
                      ...
                      console.log(
                        "🔎 CUIT heurístico detectado y aplicado:",
                        cuitMatch[0],
                      );
                    }
                  }
                  ...
                  if (
                    (!parsedData.documento ||
                      !parsedData.documento.numero ||
                      String(parsedData.documento.numero).trim().length < 2) &&
                    rawOcr
                  ) {
                    const nroMatch = rawOcr.match(
                      /(?:Nro|Nº|Numero|Factura|Núm)[:\s]*([A-Za-z0-9\-\/]+)/i,
                    );
                    if (nroMatch) {
                      parsedData.documento = {
                        ...parsedData.documento,
                      ...
                      console.log(
                        "🔎 Nro documento heurístico detectado y aplicado (primario):",
                        nroMatch[1],
                      );
                    } else {
                      // Fallback: find common invoice/folio patterns like 0400-00172176 or long digit runs
                      const fallbackMatch = rawOcr.match(
                        /(\d{3,4}-\d{5,}|\d{6,}|\d{3,}-\d{3,})/,
                      );
                      if (fallbackMatch) {
                        parsedData.documento = {
                          ...parsedData.documento,
                          numero: fallbackMatch[0],
                        };
                        console.log(
                          "🔎 Nro documento heurístico detectado y aplicado (fallback):",
                          fallbackMatch[0],
                        );
                      }
                    }
                  ...
                  if (
                    (!parsedData.documento || !parsedData.documento.fecha) &&
                    rawOcr
                  ) {
                    const dateMatch = rawOcr.match(
                      /(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})/,
                    );
                    if (dateMatch) {
                      parsedData.documento = {
                      ...
                      console.log(
                        "🔎 Fecha heurística detectada y aplicada:",
                        dateMatch[1],
                      );
                    }
                  }
                } catch (quickErr) {
                  console.warn("⚠️ Heurística rápida falló:", quickErr.message);
                }
              } catch (e) {
                console.warn("⚠️ Error preparando OCR heurístico:", e.message);
              }
              ...
              try {
                ...
                if (errors.length) {
                  ...
                  if (heur) {
                    ...
                    if (
                      (!parsedData.emisor ||
                        !parsedData.emisor.nombre ||
                        String(parsedData.emisor.nombre).trim().length < 3) &&
                      heur.emisor?.nombre
                    ) {
                      parsedData.emisor = {
                        ...parsedData.emisor,
                    ...
                    if (
                      (!parsedData.documento ||
                        !parsedData.documento.numero ||
                        String(parsedData.documento.numero).trim().length <
                          2) &&
                      heur.documento?.numero
                    ) {
                      parsedData.documento = {
                        ...parsedData.documento,
                        ...heur.documento,
                    ...
                    if (
                      errors.includes("items_missing") ||
                      errors.includes("items_bad")
                    ) {
                      parsedData.items = (heur.items || []).map((it, i) => ({
                        ordenEnFactura: i + 1,
                        descripcion_exacta: String(
                          it.descripcion_exacta || it.descripcion_limpia || "",
                        ).trim(),
                        descripcion_limpia: String(
                          it.descripcion_limpia || it.descripcion_exacta || "",
                        ).trim(),
                        cantidad_documento:
                          Number(
                            fixArgentineNumber(
                              it.cantidad_documento || it.cantidad || 0,
                            ),
                          ) || 0,
                        precio_unitario:
                          Number(
                        ...
                        subtotal_original:
                          Number(
                            fixArgentineNumber(
                              it.subtotal_original || it.subtotal || 0,
                            ),
                          ) || 0,
                        subtotal_calculado:
                          Number(
                        ...
                        descuento:
                          Number(fixArgentineNumber(it.descuento || 0)) || 0,
                        impuestos:
                          Number(fixArgentineNumber(it.impuestos || 0)) || 0,
                        es_devolucion: !!it.es_devolucion,
                        presentacion_normalizada:
                          it.presentacion_normalizada || "",
                        presentacion_candidates:
                          it.presentacion_candidates || [],
                        producto_candidates: it.producto_candidates || [],
                      }));
                      parsedData._heuristic_applied = true;
                      console.log(
                        "✅ Items reemplazados por heurística (count=" +
                  ...
                  if (post.length) {
                    ...
                    parsedData._raw_ocr =
                      vdata?.ocr_text || data?.vision_meta?.ocr_text || "";
                    ...
                    try {
                      await guardarAuditoriaIaFailure({
                        ...
                        fileSize: image.size,
                        responseStatus: 200,
                        errorText: `Validation issues: ${errors.join(",")}`,
                        timing: { before: tBeforeVision, after: Date.now() },
                      });
                    } catch (audErr) {
                      console.warn(
                        "⚠️ No se pudo registrar auditoría de validación:",
                        audErr.message,
                      );
                    }
                    ...
                    try {
                      ...
                      if (
                        (model || "").toLowerCase() !== heavyModel.toLowerCase()
                      ) {
                        console.info(
                          `🔁 Reintento con modelo pesado ${heavyModel} para mejorar extracción`,
                        );
                        ...
                        try {
                          ...
                          let b;
                          try {
                            b = new Blob([optBuf], { type: "image/jpeg" });
                          } catch (e) {
                            b = optBuf;
                          }
                          try {
                            ...
                            retryForm.append("image", b);
                          }
                        } catch (e) {
                          console.warn(
                            "⚠️ No se pudo preparar imagen para reintento:",
                            e.message,
                          );
                        }
                        ...
                        try {
                          retryForm.append(
                            "prompt",
                            prompt +
                              "\n\n// Reintento con modelo pesado para mejorar campos críticos",
                          );
                        } catch (e) {}
                        ...
                        try {
                          ...
                          console.warn(
                            "⚠️ Reintento con modelo pesado falló al conectar a vision:",
                            fetchErr.message,
                          );
                        }
                        ...
                        if (retryResp && retryResp.ok) {
                          ...
                          if (rv) {
                            ...
                            const newText = rv.extraction
                              ? typeof rv.extraction === "string"
                                ? rv.extraction
                                : JSON.stringify(rv.extraction)
                              : rv.ocr_text || "";
                            const jsonMatch2 = (newText || "").match(
                              /\{[\s\S]*\}/,
                            );
                            if (jsonMatch2) {
                              try {
                                ...
                                console.log(
                                  "✅ Reintento con modelo pesado produjo JSON parseable",
                                );
                              } catch (eJson) {
                                console.warn(
                                  "⚠️ Reintento con modelo pesado devolvió JSON inválido:",
                                  eJson.message,
                                );
                              }
                            }
                          ...
                          console.warn(
                            "⚠️ Reintento con modelo pesado no devolvió éxito",
                          );
                        }
                      }
                    } catch (errRetry) {
                      console.warn(
                        "⚠️ Error durante reintento con modelo pesado:",
                        errRetry.message,
                      );
                    }
                  } else {
                ...
                console.warn(
                  "⚠️ Error durante validación/heurística:",
                  vErr.message,
                );
              }
            }
            ...
            console.log(
              "✅ Números corregidos:",
              JSON.stringify(parsedData.totales),
            );
            console.log(
              "📦 Items finales:",
            ...
            if (parsedData.documento) {
              const pagoEstado =
                parsedData.documento.estado_pago ||
                parsedData.documento.pago_estado ||
                (parsedData.pago &&
                  (parsedData.pago.estado || parsedData.pago.estado_pago)) ||
                null;
              if (pagoEstado)
                parsedData.documento.estado_pago =
                  String(pagoEstado).toUpperCase();

              const montoPagadoRaw =
                parsedData.documento.monto_pagado ??
                (parsedData.pago
                  ? (parsedData.pago.monto_pagado ?? parsedData.pago.monto)
                  : null);
              if (
                montoPagadoRaw !== null &&
                montoPagadoRaw !== undefined &&
                montoPagadoRaw !== ""
              ) {
                const num = Number(montoPagadoRaw);
                parsedData.documento.monto_pagado = Number.isFinite(num)
                  ? fixArgentineNumber(num)
                  : parsedData.documento.monto_pagado;
              }

              const cuentaCorriente =
                parsedData.documento.cuenta_corriente ||
                parsedData.cuenta_corriente ||
                (parsedData.pago && parsedData.pago.cuenta_corriente) ||
                null;
              if (cuentaCorriente)
                parsedData.documento.cuenta_corriente = String(cuentaCorriente);

              // Inferir si está pagada (monto_pagado >= total) o si estado contiene 'PAGAD'
              if (
                typeof parsedData.documento.monto_pagado === "number" &&
                parsedData.totales &&
                typeof parsedData.totales.total === "number"
              ) {
                parsedData.documento.pagado =
                  parsedData.documento.monto_pagado >=
                  parsedData.totales.total - 0.01;
              } else if (parsedData.documento.estado_pago) {
                parsedData.documento.pagado = /PAGAD/i.test(
                  parsedData.documento.estado_pago,
                );
              }
    ...
    const timing = {
      totalMs: tEnd - tStart,
      visionMs:
        typeof tBeforeVision !== "undefined" &&
        typeof tAfterVision !== "undefined"
          ? tAfterVision - tBeforeVision
          : undefined,
      parseMs:
        typeof tAfterParsing !== "undefined"
          ? tAfterParsing -
            (typeof tAfterVision !== "undefined" ? tAfterVision : tStart)
          : undefined,
    ...
    return NextResponse.json(
      {
        ...
        metadata: {
          timing: {
            totalMs: typeof tStart !== "undefined" ? tNow - tStart : undefined,
            human:
              typeof tStart !== "undefined" ? `${tNow - tStart}ms` : undefined,
          },
        },
      },
      { status: 500 },
