...
export default function ImageCropper({
  ...
  const handleMouseDown = (e) => {
    ...
    for (let corner of corners) {
      if (
        Math.abs(x - corner.x) <= cornerHitSize &&
        Math.abs(y - corner.y) <= cornerHitSize
      ) {
        setIsDragging(true);
    ...
    if (
      Math.abs(x - (crop.x + crop.width / 2)) <= edgeHitSize &&
      Math.abs(y - crop.y) <= edgeHitSize
    ) {
      setIsDragging(true);
    ...
    if (
      Math.abs(x - (crop.x + crop.width / 2)) <= edgeHitSize &&
      Math.abs(y - (crop.y + crop.height)) <= edgeHitSize
    ) {
      setIsDragging(true);
    ...
    if (
      Math.abs(y - (crop.y + crop.height / 2)) <= edgeHitSize &&
      Math.abs(x - crop.x) <= edgeHitSize
    ) {
      setIsDragging(true);
    ...
    if (
      Math.abs(y - (crop.y + crop.height / 2)) <= edgeHitSize &&
      Math.abs(x - (crop.x + crop.width)) <= edgeHitSize
    ) {
      setIsDragging(true);
  ...
  const handleMouseMove = (e) => {
    ...
    if (dragType?.startsWith("guide-corner-")) {
      ...
      const updatedGuide = resizeGuideFromCorner(
        guides[dragGuideIndex],
        cornerName,
        x,
        y,
        crop,
    ...
    // Manejo del crop principal
      ...
      newCrop.x = Math.max(
        0,
        Math.min(canvasRef.current.width - crop.width, crop.x + dx),
      );
      newCrop.y = Math.max(
        0,
        Math.min(canvasRef.current.height - crop.height, crop.y + dy),
      );
      setCrop(newCrop);
      setDragStart({ x, y });
      ...
      if (dragType.includes("s")) {
        newCrop.height = Math.max(
          20,
          Math.min(canvasRef.current.height - crop.y, y - crop.y),
        );
      }
      ...
      if (dragType.includes("e")) {
        newCrop.width = Math.max(
          20,
          Math.min(canvasRef.current.width - crop.x, x - crop.x),
        );
      }
            ...
            if (mode === "factura") {
              ...
              Object.keys(mapping).forEach((key) => {
                if (detected[key] && newGuides[mapping[key]]) {
                  ...
                  newGuides[mapping[key]].y =
                    crop.y + (crop.height * d.y) / 100;
                  newGuides[mapping[key]].width = (crop.width * d.width) / 100;
                  newGuides[mapping[key]].height =
                    (crop.height * d.height) / 100;
                }
              });
            } else if (mode === "producto") {
              const mapping = {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-[95vw] h-[95vh] flex flex-col">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
          <div>
            <h3 className="font-semibold text-lg text-gray-900">
              ✂️ Recortar imagen y definir regiones
            </h3>
            <p className="text-sm text-gray-600 mt-1">
              <span className="font-medium">💡 Controles:</span>
              Arrastra los{" "}
              <span className="font-semibold text-purple-600">
                puntos de control (círculos)
              </span>{" "}
              en las esquinas para ajustar campos • Marco azul para ajustar el
              recorte general • Click fuera no hace nada
            </p>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          <div
            className="bg-gray-900 rounded-lg flex justify-center items-center min-h-0"
