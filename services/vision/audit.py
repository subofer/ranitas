...
def audit(task):
  def deco(fn):
    ...
    async def wrapper(*a, **kw):
      import state
      t0 = perf_counter()
      state.state['counters']['tasks'].setdefault(task,0); state.state['counters']['tasks'][task]+=1
      try:
        ...
        state.state['audit'] = {'last_op':task,'last_status':'ok','last_latency_ms':dur}
        ...
        state.state['audit'] = {'last_op':task,'last_status':'error','last_latency_ms':dur,'last_error':tb}
