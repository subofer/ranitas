  ...
  return (
    ...
    <>
      ...
      {presentacionesOrdenadas.map((pres) => {
        ...
        const esPresSeleccionada = presentacionSeleccionadaPorProducto?.[productoId] === pres.id;

        const bgPres = filaPresActiva ? 'bg-blue-200' : (esFilaFocused ? 'bg-blue-100' : (esPresSeleccionada ? 'bg-blue-50' : ''));
        ...
        return (
          <tr key={rowId} className="group border-b bg-white hover:bg-blue-50 transition-colors">
            <td className="px-3 py-2 sticky left-0 z-10 bg-white group-hover:bg-blue-50"> 
              <input
                type="checkbox"
                checked={esPresSeleccionada}
                onChange={() => onSetPresentacionSeleccionada?.(productoId, esPresSeleccionada ? null : pres.id)}
                className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            </td>

            <td className="px-3 py-2 text-sm bg-white group-hover:bg-blue-50 select-text">
                ...
                {!editable || !filaPresActiva ? (
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <input
                      ...
                      className="w-28 px-2 py-1 border border-gray-300 rounded text-xs bg-white text-right"
                      placeholder="Desc %"
                      title="Descuento (%)"
                    />
                  </div>
            ...
            <td className="px-3 py-2 text-sm sticky right-0 z-10 bg-white group-hover:bg-blue-50">
              <div className="flex items-center justify-end gap-1">
