...
export async function loadOpenCV() {
  ...
  cvLoadPromise = new Promise((resolve, reject) => {
    ...
    // Crear script tag para cargar OpenCV.js desde el bundle local en public/opencv/opencv.js
    // Asegúrate de tener este archivo disponible en producción (scripts/download-opencv.sh puede ayudar)
    ...
    script.onload = () => {
      ...
      const checkOpenCV = setInterval(() => {
        if (window.cv && window.cv.Mat) {
          ...
          cv = window.cv
          console.log('✅ OpenCV.js (local) cargado correctamente')
          resolve(cv)
        }
      }, 100)
      ...
      setTimeout(() => {
        clearInterval(checkOpenCV)
        if (!cv) {
          reject(new Error('Timeout cargando OpenCV.js (local)'))
        }
      }, 10000)
    }
    
    script.onerror = () => {
      reject(new Error('Error cargando OpenCV.js local. Asegúrate de tener `public/opencv/opencv.js` (ejecuta `scripts/download-opencv.sh`)'))
    }
